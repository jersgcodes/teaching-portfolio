# Source notes — AI Fluency for Educators

> **What this is.** Notes from a video course on AI Fluency aimed at educators, focused on applying the 4D framework (Delegation, Description, Discernment, Diligence) to teaching practice. Source material — not portfolio content. Lives here so future portfolio writing can draw from a single canonical reference rather than re-summarising each time.
>
> **Source.** Anthropic course about working with Claude — specifically the educator-targeted track.
>
> **Captured.** 2026-05-07.
>
> **Use.** Reference for any portfolio content that touches educator audiences (e.g. a future `/educators` content collection on growth, or labs deliverables aimed at teaching audiences). Also informs our 5-stage workflow's framing — see the crosswalk below.

---

## Crosswalk: academic 4D ↔ this portfolio's 5-stage workflow

The notes use the academic 4D framework. Our locked taxonomy (see `taxonomy/workflow-5stage.json`, ADR 0003) uses a 5-stage workflow that adds two bookends. The mapping:

| Academic 4D | Our 5-stage | Note |
|---|---|---|
| — | **Define** | Added — explicit front-end framing of the problem and success criteria, before any AI involvement |
| **Delegation** | **Delegate** | Same concept — deciding what AI does, what stays human |
| **Description** | **Direct** | Same concept; we use "Direct" to suggest active steering rather than just communication. Encompasses product, process, and performance description from the notes |
| **Discernment** | **Discern** | Same concept — evaluating output across product, process, and performance dimensions |
| **Diligence** | (partly **Document**, partly cross-cutting) | Diligence is broader than Document — covers transparency, accountability, ethical use throughout the cycle. We capture the recording / attribution / decision-log piece in Document; the in-the-loop responsibility piece is treated as cross-cutting through every stage |
| — | **Document** | Added — explicit tail-end capture of decisions, lessons, attribution. Closes the loop and makes the next cycle better |

For educator audiences accustomed to 4D, the 5-stage framing should be presentable as "4D plus two bookends — Define before, Document after." Both frameworks describe the same underlying cognitive partnership; the 5-stage version is more explicit about the pedagogical-style "preparation and reflection" beats that bookend the AI engagement.

---

## How these notes will be used (when relevant)

- **`/educators` content collection (future, deferred to v1.5+ of growth surface).** Course design + learning materials + assignment design as worked examples drawn from the lessons below.
- **`/ai-fluency` page voice.** The educator-style "thinking partner, not tool" framing aligns with our portfolio voice; specific phrasings (especially "augmentation over automation", "responsible human in the loop") are quotable.
- **Cross-project pattern: AI role-playing technique.** The "ask AI to act as one of my students" technique from Lesson 2 is a generalisable cognitive partnership move worth lifting into a future pattern (`content/patterns/ai-as-perspective-taker.md` candidate).
- **Teaching Context Document concept.** The reusable shared-context artifact described in Lesson 1 maps directly onto our `~/claude/<slug>/project-overview.yaml` standard. Worth name-checking the parallel.
...
**Delegation** = determining who does what work (human vs AI vs collaborative). Three types:
...
**Description** = how to communicate with AI (conversation not commands). Three dimensions:
...
**Discernment** = evaluating AI outputs and adjusting accordingly. Three levels:
...
**Diligence** = responsible AI use throughout collaboration. Three aspects:
...
Reusable artifact establishing shared understanding with AI for future collaborations.
...
> *Portfolio note:* the Teaching Context Document concept maps directly onto our `project-overview.yaml` workspace standard — same shape, same purpose, different vocabulary. Worth name-checking the parallel when we write educator content.
...
- Don't just accept/reject — explain why something works or doesn't in context
...
> *Portfolio note:* this technique generalises beyond education — "act as a sceptical reader / a junior dev / a regulator" is the same move. Worth lifting into a `content/patterns/ai-as-perspective-taker.md` pattern.
...
3. *Who's in charge* — how humans manage and remain accountable for AI work
...
- *Description-Discernment Loop* — refinement tool, explain why suggestions work or don't work
